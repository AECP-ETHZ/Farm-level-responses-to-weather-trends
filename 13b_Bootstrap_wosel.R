#-----------------------------------------------------#
#                                                     #
# This program estimates the model without non-random #
# crop selection with bootstrapped standard errors.   #
#                                                     #
...
# stored in lines 700 and 715, allowing to            # 
...
#--------------------------------#
  ...
  for (i in 1:n_qcrops) { 
    ...
    for (j in 1:n_prices) {
      ...
      coef_struct_p <- coefs_struct_crop %>% select(ends_with(list_prices[j,"prices"]))
      mean_price <- dat_sm %>% select(list_prices[j,"prices"])
      
      elast_prices[j,1+i] <- (coef_struct_p) * (mean_price / mean_quantity )  
    }
  ...
  # ---------------------------------- #  
    ...
    for (i in 1:n_qcrops) { 
      ...
      for (j in 1:n_weather) {
        ...
        elast_weather[j,i+1] <-(coef_struct_weather + coef_struct_weather_obs_1to3*mean_weather_1to3 + coef_struct_weather_obs_4to10*mean_weather_4to10)  * (1/mean_quantity)
        
        # Weather_1to3
        coef_struct_weather <- coefs_struct_crop %>% select(ends_with(list_weather[j,"weather_1to3_endwith"]))
        
        elast_weather_1to3 <- (coef_struct_weather + coef_struct_weather_obs_1to3*mean_weather_obs) * (1 / mean_quantity )
        
        # Weather_4to10
        coef_struct_weather <- coefs_struct_crop %>% select(ends_with(list_weather[j,"weather_4to10_endwith"]))
        
        elast_weather_4to10 <- (coef_struct_weather + coef_struct_weather_obs_4to10*mean_weather_obs) * (1 / mean_quantity )
  ...
  # ------------------------------- #    
    ...
    }, error=function(err) {rep(NA,475)} ) # insert NA's if there is an optimization error
  }
...
# save actual result
save(boot_actresults, file="rOutput/bootresults_wosel_actual.Rda")
...
# save
save(boot_results, file="rOutput/bootresults_linear_wosel.Rda")
boot_results <- NULL
...
load("rOutput/bootresults_wosel.Rda")
load("rOutput/bootresults_wosel_actual.Rda")
...
# Print
write_xlsx(tab_structural,"Tables/Table_S8.1.xlsx")
...
# Print
write_xlsx(tab_PriceElast,"Tables/Table_S8.2.xlsx")
...
# Print
write_xlsx(tab_WeatherElast,"Tables/Table_S8.3.xlsx")
