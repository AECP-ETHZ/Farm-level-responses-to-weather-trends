...
# Add observed weather to simulated weather
  ...
  sim2018$gdd_obs <- c(df_weather_lta$gdd, df_weather_2018$gdd, rep(df_weather_lta$gdd,10))
  sim2018$prec_obs <- c(df_weather_lta$prec, df_weather_2018$prec, rep(df_weather_lta$prec,10))
  sim2018$gddHigh_obs <- c(df_weather_lta$gddHigh, df_weather_2018$gddHigh, rep(df_weather_lta$gddHigh,10))
...
model_linear <- systemfit( formula = system, method = "SUR", 
    ...
    predicted_outcomes <- predict(model_linear)
    caret::postResample(df_farm$qq_cereals,predicted_outcomes$QQcereals.pred)
    caret::postResample(df_farm$qq_protein,predicted_outcomes$QQprotein.pred)
    caret::postResample(df_farm$qq_oilseed,predicted_outcomes$QQoilseed.pred)
    caret::postResample(df_farm$qq_roots,predicted_outcomes$QQroots.pred)
    caret::postResample(df_farm$qq_corn,predicted_outcomes$QQcorn.pred)
    caret::postResample(df_farm$x_fert,-predicted_outcomes$NXfert.pred)
...
# ------------------------------------ #
  ...
  save(simresults, file="rOutput/simresults_main.Rda")
...
#----------------------------------------------#
  ...
  # Load results
  load("rOutput/simresults_main.Rda")
  ...
  # Plot the results
  Fig_main <- ggplot(simresults_long, aes(x=t, y=value, group=variable, shape=variable, color=variable)) +
    geom_line()+
    geom_point() +
    theme_bw() +
  ...
  ggsave("Figures/Figure_3.png", Fig_main, device="png", width = 6, height = 3, units = "in", dpi=1200)
  ggsave("Figures/Figure_3.eps", Fig_main, device="eps", width = 6, height = 3, units = "in", dpi=1200)
