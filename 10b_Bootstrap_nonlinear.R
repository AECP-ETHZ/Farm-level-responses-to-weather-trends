#------------------------------------------------------#
#                                                      #
# This program estimates the model with nonlinearities #
# in weather with bootstrapped standard errors.        #
#                                                      #
...
# stored in lines 924 and 939, allowing to             # 
...
#----------------------------------------#
  ...
  model_nonlinear <- systemfit::systemfit( formula = system, method = "SUR", 
                         data = data_boot, restrict.matrix = restrict, 
                         maxit = 100 )
    
  coef_nonlinear <- coef(model_nonlinear)  
  
  # "x times \beta" (needed for elasticities)
  qx_pred <- predict(model_nonlinear)
  
  qq_cereals_pred_b <- mean(qx_pred$QQcereals.pred) - model_nonlinear$coefficients["QQcereals_phi_cereals"]*mean(data_boot$phi_cereals) 
  qq_cereals_pred <- (qq_cereals_pred_b) / mean(data_boot$PHI_cereals)
  
  qq_protein_pred_b <- mean(qx_pred$QQprotein.pred) - model_nonlinear$coefficients["QQprotein_phi_protein"]*mean(data_boot$phi_protein) 
  qq_protein_pred <- (qq_protein_pred_b) / mean(data_boot$PHI_protein)
  
  qq_oilseed_pred_b <- mean(qx_pred$QQoilseed.pred) - model_nonlinear$coefficients["QQoilseed_phi_oilseed"]*mean(data_boot$phi_oilseed) 
  qq_oilseed_pred <- (qq_oilseed_pred_b) / mean(data_boot$PHI_oilseed)
  
  qq_roots_pred_b <- mean(qx_pred$QQroots.pred) - model_nonlinear$coefficients["QQroots_phi_roots"]*mean(data_boot$phi_roots) 
  qq_roots_pred <- (qq_roots_pred_b) / mean(data_boot$PHI_roots)
  
  qq_corn_pred_b <- mean(qx_pred$QQcorn.pred) - model_nonlinear$coefficients["QQcorn_phi_corn"]*mean(data_boot$phi_corn) 
  ...
  # ---------------------------------#
    ...
    for (i in 1:n_qcrops) { 
      ...
      coefs_struct_crop <- dplyr::as_tibble(t(coef_nonlinear)) %>% select(starts_with(list_qcrops[i,"struct"])) 
    ...
    coef_fert <- dplyr::as_tibble(t(coef_nonlinear)) %>% select(starts_with("NXfert")) 
  ...
  # ---------------------------------- #  
    ...
    list_weather <- data.frame(weather_obs=  c("gdd_obs","prec_obs","gddHigh_obs","dd_obs"),
                               weather2_obs = c("gdd2_obs", "prec2_obs", NA, NA),
                               weather_1to3= c("gdd_1to3","prec_1to3","gddHigh_1to3","dd_1to3"),
                               weather2_1to3 = c("gdd2_1to3", "prec2_1to3", NA, NA),
                               weather_4to10=c("gdd_4to10","prec_4to10","gddHigh_4to10","dd_4to10"),
                               weather2_4to10 = c("gdd2_4to10", "prec2_4to10", NA, NA),
                               ...
                               weather2_obs_endwith = c("* gdd2_obs)", "* prec2_obs)", NA, NA),
                               weather_1to3_endwith= c("* gdd_1to3)","* prec_1to3)","* gddHigh_1to3)","* dd_1to3)"),
                               weather2_1to3_endwith = c("* gdd2_1to3)", "* prec2_1to3)", NA, NA),
                               weather_4to10_endwith=c("* gdd_4to10)","* prec_4to10)","* gddHigh_4to10)","* dd_4to10)"),
                               weather2_4to10_endwith = c("* gdd2_4to10)", "* prec2_4to10)", NA, NA),
    ...
    for (i in 1:n_qcrops) { 
      ...
      for (j in 1:n_weather) {
        ...
        # Weather-squared terms
        coef_probit_weather2_1to3 <- as.numeric(ifelse(j<3,coefs_probit_crop %>% select(list_weather[j,"weather2_1to3"]),0)) #squared terms only for gdd and prec
        coef_probit_weather2_4to10 <-  as.numeric(ifelse(j<3,coefs_probit_crop %>% select(list_weather[j,"weather2_4to10"]),0))
        
        coef_struct_weather2_obs <-  as.numeric(ifelse(j<3,coefs_struct_crop %>% select(ends_with(list_weather[j,"weather2_obs_endwith"])),0)) #squared terms only for gdd and prec
        coef_struct_weather2_1to3 <- as.numeric(ifelse(j<3,coefs_struct_crop %>% select(ends_with(list_weather[j,"weather2_1to3_endwith"])),0))
        coef_struct_weather2_4to10 <-  as.numeric(ifelse(j<3,coefs_struct_crop %>% select(ends_with(list_weather[j,"weather2_4to10_endwith"])),0))
        ...
        elast_weather[j,i+1] <- PHI * (coef_struct_weather + coef_struct_weather_obs_1to3*mean_weather_1to3 + coef_struct_weather_obs_4to10*mean_weather_4to10 + 2*coef_struct_weather2_obs*mean_weather_obs)  * (1/mean_quantity)
        ...
        elast_weather_1to3 <- (PHI * (coef_struct_weather + coef_struct_weather_obs_1to3*mean_weather_obs + 2*coef_struct_weather2_1to3*mean_weather_1to3) + phi * pred * 
                                 (coef_probit_weather + 2*coef_probit_weather2_1to3*mean_weather_1to3) -
                                 coef_phi_struct * linpred * (coef_probit_weather + 2*coef_probit_weather2_1to3*mean_weather_1to3) * phi) * (1 / mean_quantity )
        ...
        elast_weather_4to10 <- (PHI * (coef_struct_weather + coef_struct_weather_obs_4to10*mean_weather_obs + 2*coef_struct_weather2_4to10*mean_weather_4to10) + phi * pred * 
                                  (coef_probit_weather + 2*coef_probit_weather2_4to10*mean_weather_4to10) -
                                  coef_phi_struct * linpred * (coef_probit_weather + 2*coef_probit_weather2_4to10*mean_weather_4to10) * phi) * (1 / mean_quantity )
    ...
    list_weather <- data.frame(weather_obs=  c("gdd_obs","prec_obs","gddHigh_obs","dd_obs"),
                               ...
                               weather2_obs_endwith = c("_gdd2_obs", "_prec2_obs", NA, NA),
                               weather_1to3_endwith= c("_gdd_1to3","_prec_1to3","_gddHigh_1to3","_dd_1to3"),
                               weather2_1to3_endwith = c("_gdd2_1to3", "_prec2_1to3", NA, NA),
                               weather_4to10_endwith=c("_gdd_4to10","_prec_4to10","_gddHigh_4to10","_dd_4to10"),
                               weather2_4to10_endwith = c("_gdd2_4to10", "_prec2_4to10", NA, NA),
    ...
    coefs_fert <- dplyr::as_tibble(t(coef_nonlinear)) %>% select(starts_with("NXfert")) 
    ...
    for (j in 1:n_weather) {
      ...
      # Weather-squared terms
      coef_fert_weather2_obs <-  as.numeric(ifelse(j<3,coefs_fert %>% select(ends_with(list_weather[j,"weather2_obs_endwith"])),0)) #squared terms only for gdd and prec
      coef_fert_weather2_1to3 <- as.numeric(ifelse(j<3,coefs_fert %>% select(ends_with(list_weather[j,"weather2_1to3_endwith"])),0))
      coef_fert_weather2_4to10 <-  as.numeric(ifelse(j<3,coefs_fert %>% select(ends_with(list_weather[j,"weather2_4to10_endwith"])),0))
      ...
      elast_weather[j,7] <- (coef_fert_weather + coef_fert_weather_obs_1to3*mean_weather_1to3 + coef_fert_weather_obs_4to10*mean_weather_4to10 + 2*coef_fert_weather2_obs*mean_weather_obs) * (1/dat_sm$nx_fert)
      
      # Weather_1to3
      coef_fert_weather <- coefs_fert %>% select(ends_with(list_weather[j,"weather_1to3_endwith"]))
      elast_weather_1to3 <- (coef_fert_weather + coef_fert_weather_obs_1to3*mean_weather_obs + 2*coef_fert_weather2_1to3*mean_weather_1to3) * (1/dat_sm$nx_fert)
      
      # Weather_4to10
      coef_fert_weather <- coefs_fert %>% select(ends_with(list_weather[j,"weather_4to10_endwith"]))
      elast_weather_4to10 <- (coef_fert_weather + coef_fert_weather_obs_4to10*mean_weather_obs + 2*coef_fert_weather2_4to10*mean_weather_4to10) * (1/dat_sm$nx_fert)
  ...
  # ------------------------------- #    
    ...
    result <- c(coef_iCereals, # Probit coefficients
                ...
                iRoots_me,
                iCorn_me,
                model_nonlinear$coefficients, #structural coefficients
    ...
    }, error=function(err) {rep(NA,1102)} ) # insert NA's if there is an optimization error
  }
...
# save actual result
save(boot_actresults, file="rOutput/bootresults_nonlinear_actual.Rda")
...
# save
save(boot_results, file="rOutput/bootresults_nonlinear.Rda")
boot_results <- NULL
...
load("rOutput/bootresults_nonlinear.Rda")
load("rOutput/bootresults_nonlinear_actual.Rda")
...
# Organize
coef_iCereals <- boot_summary[c(1:55),]
coef_iProtein <- boot_summary[c(56:110),]
coef_iOilseed <- boot_summary[c(111:165),]
coef_iRoots <- boot_summary[c(166:220),]
coef_iCorn <- boot_summary[c(221:275),]
iCereals_me <- boot_summary[c(276:330),]
iProtein_me <- boot_summary[c(331:385),]
iOilseed_me <- boot_summary[c(386:440),]
iRoots_me <- boot_summary[c(441:495),]
iCorn_me <- boot_summary[c(496:550),]
...
tab_parprobit <- tab_parprobit[1:28,]
...
# Print
write_xlsx(tab_parprobit,"Tables/Table_S6.4.1.xlsx")
...
tab_structural$Variable <- c("Intercept", "P Cereals", "P Protein", "P Oilseed crops", "P Root crops", "P Corn", "R Fert", 
                             "K Land", "K Labor", "K Capital", "Trend", "Trend^2", 
                             "Gdd", "Gdd2", "Prec", "Prec2", "GddHigh", "DD",
                             "Gdd_1to3", "Gdd2_1to3", "Prec_1to3", "Prec2_1to3", "GddHigh_1to3", "DD_1to3",
                             "Gdd_4to10", "Gdd2_4to10", "Prec_4to10", "Prec2_4to10", "GddHigh_4to10", "DD_4to10",
...
# Print
write_xlsx(tab_structural,"Tables/Table_S6.4.2.xlsx")
...
# Print
write_xlsx(tab_PriceElast,"Tables/Table_S6.4.3.xlsx")
...
# Print
write_xlsx(tab_WeatherElast,"Tables/Table_S6.4.4.xlsx")
